...
public class Polynomial {
    ...
    public static synchronized void LOG(String s)//lightly modify 
    {
    ...
    public synchronized int solve(double min, double max, int nthreads, double slices, double precision) ///modify
    {
    	nthreads=Intiger.parseInt(nthreads);
        
        for(int i=0;i<nthreads;i++)
        {
            final int threadID=i;
            thread[i]= new thread(()->pool.thread(threadID));
            thread[i].start;
        } 
        for(int i=0;i<nthreads;i++)
        {
    ...
    private synchronized int solveRecursive(double min, double max, int threadID, double slices, double precision, int recursion) ///lightly modify
    {
